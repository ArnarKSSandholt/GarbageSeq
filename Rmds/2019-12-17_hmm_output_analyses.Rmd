...
Lastly, we'll just write the cluster files:
...
# Searching further on viral HMM profiles

```{r}
#entrez_dbs()
#entrez_db_searchable(db = "taxonomy")
res <- entrez_search(db = "taxonomy", term = "(PLoS Neglected Tropical Diseases[JOUR] AND 2015[PDAT])", retmax = 9999, use_history = T)
res$count
#res$ids
...
```{r}
#a <- xpathSApply(recs, "//MedlineCitation/Article/ArticleTitle", xmlValue)
#esums <- entrez_summary(db = "pubmed", id = res$ids[1:25])
a <- extract_from_esummary(esums, "title")
#entrez_info(db = "mesh")
a
```
