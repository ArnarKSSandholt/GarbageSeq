...
for TAXID in $DATA*/ ; do
  ...
  for HMM in "${HMMs[@]}"; do
    ...
    for fOTUproteom in "${fOTUproteoms[@]}"; do
      ...
      echo $fOTU,$HMM_VARIANT,$TAXID_NO >> hmmruns.csv
      #hmmsearch --cpu 4 --noali --notextw -E 0.01 --domE 0.01 "$HMM" "$fOTUproteom" >> "$HMMer_OUTPUT_DIR""$TAXID_NO""-with-""$HMM_VARIANT""-in-""$fOTU"".out"
